...
//		int updateCount = 0;
		...
		try(Connection conn = DBConnectionFactory.getOracleDBConnection();){
			...
			try(PreparedStatement stmt = conn.prepareStatement(querySQL)){
				stmt.setBigDecimal(1, goods.getGoodsID());	
	...
	public boolean createGoods(Goods goods){
		...
		try(Connection conn = DBConnectionFactory.getOracleDBConnection();){
			...
			try(PreparedStatement stmt = conn.prepareStatement(insertSQL)){
				...
				//stmt.setString(5, goods.getGoodsImageName()== null ? "default.jpg" : goods.getGoodsImageName());
	...
	public boolean deleteGoods(BigDecimal goodsID){
		...
		try (Connection conn = DBConnectionFactory.getOracleDBConnection();){
			conn.setAutoCommit(false);
			String insertSQL = "DELETE FROM BEVERAGE_GOODS WHERE GOODS_ID = ?";
			try (PreparedStatement stmt = conn.prepareStatement(insertSQL)){
	...
	public List<SalesReport> querySalesReport(String startDate, String endDate) {
	    ...
	    querySQL.append("AND O.ORDER_DATE BETWEEN TO_DATE(?, 'YYYY-MM-DD') AND TO_DATE(?, 'YYYY-MM-DD')");
